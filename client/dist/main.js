(() => {
  ...
  async function renderTicketView(app, state2, navigateTo2) {
    state2.role = "customer";
    const reservationId = state2?.params?.reservationId;
    if (!reservationId) {
      app.innerHTML = `
      <section>
        <h2>Ticket</h2>
        <p>Keine Reservierungs-ID vorhanden.</p>
        <button type="button" id="back">Zur\xFCck</button>
      </section>
    `;
      const backBtn2 = document.getElementById("back");
      if (backBtn2) {
        backBtn2.addEventListener("click", () => navigateTo2("customerHome"));
      }
      return null;
    }
    app.innerHTML = `
  ...
  `;
    ...
    if (backBtn) {
      backBtn.addEventListener("click", () => navigateTo2("customerHome"));
    }
  ...
  function renderHomeView(app, navigateTo2, state2, setRole2) {
    app.innerHTML = `
    <section class="role-selection">
      <button type="button" class="role-card" id="choose-customer">
        ...
        <div class="role-card-text">Vorstellungen ansehen, Pl\xE4tze w\xE4hlen und dein Ticket mit QR-Code sichern.</div>
      ...
      <button type="button" class="role-card" id="choose-operator">
        ...
        <div class="role-card-text">Kinos\xE4le anlegen, Shows planen und den Betrieb im Blick behalten.</div>
  ...
  `;
    ...
    if (btnCustomer) {
      btnCustomer.addEventListener("click", () => setRole2("customer"));
    }
    const btnOperator = document.getElementById("choose-operator");
    if (btnOperator) {
      btnOperator.addEventListener("click", () => setRole2("operator"));
    }
    if (state2?.role) {
      state2.role = null;
    }
  ...
  function createResponsivePaginator({
    listEl,
    controlsEl,
    renderItem,
    getItems,
    ...
    if (!pageLabelEl || !prevBtn || !nextBtn) {
      throw new Error("Pagination-Controls unvollst\xE4ndig.");
    }
    ...
    const onResize = debounce(render2, 150);
  ...
  async function renderCustomerHomeView(app, navigateTo2, state2) {
    state2.role = "customer";
    app.innerHTML = `
    <section>
      <h2>Kunde</h2>
      <p>W\xE4hle eine Vorstellung und reserviere freie Sitzpl\xE4tze.</p>
  ...
  `;
    ...
    listEl.addEventListener("click", (event) => {
      ...
      if (showId) {
        navigateTo2("customerReservation", { showId });
      }
    });
    try {
  ...
  async function renderCustomerReservationView(app, navigateTo2, state2) {
    state2.role = "customer";
    const showId = state2?.params?.showId;
    if (!showId) {
      app.innerHTML = `
      <section>
        <h2>Reservierung</h2>
        <p>Bitte zuerst eine Vorstellung ausw\xE4hlen.</p>
        <button type="button" id="back">Zur\xFCck</button>
      </section>
    `;
      const backBtn2 = document.getElementById("back");
    ...
    <section>
      ...
      <div class="seat-legend">
        ...
        <span class="seat-legend-item"><span class="seat seat--selected" aria-hidden="true"></span> ausgew\xE4hlt</span>
  ...
  `;
    ...
    formEl.addEventListener("submit", async (event) => {
      ...
      try {
        ...
        messageEl.textContent = "Reserviere Sitzpl\xE4tze...";
        ...
        navigateTo2("ticket", { reservationId: String(reservationId) });
      } catch (err) {
        console.error(err);
        if (err.status === 409) {
          messageEl.textContent = "Mindestens ein Sitzplatz wurde gerade bereits reserviert. Bitte neu ausw\xE4hlen.";
          selected.clear();
          try {
            const occupied = await apiGet(`/shows/${showId}/occupied-seats`);
            occupiedSeatCodes = occupied?.seatCodes || [];
  ...
  function renderOperatorHomeView(app, navigateTo2, state2, setRole2) {
    app.innerHTML = `
    <section>
      <h2>Betreiber Bereich</h2>
      <p>Du kannst Kinos\xE4le und Vorstellungen anlegen.</p>
      <div class="home-buttons">
        <button type="button" id="go-halls">Kinos\xE4le anlegen</button>
  ...
  `;
    ...
    if (goHalls) {
      goHalls.addEventListener("click", () => navigateTo2("operatorHalls"));
    }
    const goShows = document.getElementById("go-shows");
    if (goShows) {
      goShows.addEventListener("click", () => navigateTo2("operatorShows"));
    }
    const switchRoleBtn = document.getElementById("switch-role");
    if (switchRoleBtn) {
      switchRoleBtn.addEventListener("click", () => {
    ...
    <div class="hall-modal-backdrop" data-action="close">
      <div class="hall-modal" role="dialog" aria-modal="true" aria-label="Kinosaal">
        <header class="hall-modal-header">
          ...
          <button type="button" class="hall-modal-close" data-action="close" aria-label="Schlie\xDFen">
  ...
  async function renderOperatorHallsView(app, navigateTo2, state2) {
    state2.role = "operator";
    app.innerHTML = `
    <section class="hall-section">
      <div class="hall-section-header">
        ...
        <button type="button" id="back-home">Zur\xFCck</button>
      </div>
      <p class="hall-message">Kinos\xE4le k\xF6nnen nur angelegt (nicht bearbeitet/gel\xF6scht) werden.</p>
  ...
  `;
    ...
    if (backBtn) {
      backBtn.addEventListener("click", () => navigateTo2("operatorHome"));
    }
  ...
  async function renderOperatorShowsView(app, navigateTo2, state2) {
    state2.role = "operator";
    app.innerHTML = `
    <section>
      <div class="hall-section-header">
      ...
      <p class="hall-message">Vorstellungen k\xF6nnen nur angelegt (nicht bearbeitet/gel\xF6scht) werden.</p>
  ...
  `;
    ...
    const paginator = createResponsivePaginator({
      ...
      renderItem: (show) => {
        ...
        <div class="resource-item">
          <div class="resource-main">
            <div class="resource-title">${show.movieTitle || "Unbekannter Film"}${hallName}</div>
            <div class="resource-meta">${formatDateTime2(show.startsAt)}</div>
          </div>
        </div>
      `;
      }
    ...
    try {
      ...
      if (halls.length === 0) {
        ...
        startsAtInput.value = toDatetimeLocalValue(/* @__PURE__ */ new Date());
        return () => paginator.destroy();
      }
      hallSelect.innerHTML = halls.map((hall) => `<option value="${hall._id}">${hall.name}</option>`).join("");
      startsAtInput.value = toDatetimeLocalValue(new Date(Date.now() + 60 * 60 * 1e3));
      messageEl.textContent = "Lade Vorstellungen...";
      shows = await apiGet("/shows");
      messageEl.textContent = shows.length === 0 ? "Noch keine Vorstellungen vorhanden." : "";
      paginator.render();
    ...
    formEl.addEventListener("submit", async (event) => {
      ...
      if (!hallId) {
        messageEl.textContent = "Bitte einen Kinosaal ausw\xE4hlen.";
        return;
      }
      try {
