(() => {
  ...
  function renderTicketView(app, state2, navigateTo2) {
    ...
    const qr = qrcode_default(typeNumber, errorCorrectionLevel);
  ...
  `;
    ...
    if (backButton) {
      backButton.addEventListener("click", () => navigateTo2("home"));
    }
  }
  ...
  async function fetchHalls() {
    ...
    if (!response.ok) {
      throw new Error("Fehler beim Laden der Kinos\xE4le");
    }
  ...
  async function deleteHall(id) {
    ...
    if (!response.ok && response.status !== 204) {
      throw new Error("Fehler beim L\xF6schen eines Kinosaals");
    }
  }
  function renderHallGrid(halls) {
    ...
    <button class="hall-card" type="button"
      ...
      <button class="hall-delete-button" type="button" data-id="${hall._id}" aria-label="Kinosaal l\xF6schen">
  ...
  function openHallModal(hall) {
    ...
    <div class="hall-modal-content">
      <header class="hall-modal-header">
        <h3>${hall.name}</h3>
        <button type="button" class="hall-modal-close" aria-label="Schlie\xDFen">
  ...
  async function renderOperatorHallsView(app, navigateTo2) {
    app.innerHTML = `
    ...
    if (grid) {
      grid.addEventListener("click", async (event) => {
        ...
        if (deleteButton && deleteButton.dataset.id) {
          ...
          const confirmed = window.confirm(`M\xF6chtest du den Saal "${name}" wirklich l\xF6schen?`);
          if (!confirmed) {
            return;
          }
          try {
            ...
            console.error(err);
            messageEl.textContent = "Fehler beim L\xF6schen des Saals.";
          }
          return;
        }
